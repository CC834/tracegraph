...
### Data archaeology for undocumented relational systems

TraceGraph is a visual, privacy-first tool for following one record through an unfamiliar relational database. It combines declared foreign-key evidence, optional column-match inference, bounded graph traversal, and snapshot replay to make hidden data flows explainable.

![TraceGraph animated synthetic record journey](docs/tracegraph-demo.gif)

> The public demo and every committed fixture are synthetic. TraceGraph contains no employer, client, thesis, or production database schema, records, queries, identifiers, screenshots, or benchmark results.
...
TraceGraph was inspired by database-investigation challenges encountered while contributing to a collaborative industrial MSc thesis. The original investigation required following selected records, comparing database states, and testing possible relationships before later analytics could be trusted. This public implementation independently generalizes that workflow around a deterministic e-commerce scenario.
...
Open `http://127.0.0.1:5173`. Without configuration, TraceGraph starts with its synthetic SQLite database.
...
Connection URLs are launch configuration only. They are never accepted from, returned to, or persisted by the browser. TraceGraph also enables SQLite query-only mode or PostgreSQL read-only sessions, applies a PostgreSQL statement timeout, validates identifiers against inspected metadata, and exposes no arbitrary-SQL endpoint.
...
- TraceGraph is an investigation aid; conclusions still require domain knowledge and validation.
